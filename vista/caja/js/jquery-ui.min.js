...
(function($){
    ...
    $.fn.extend({
       validate: function(options){
           ...
           function mayor (val,title) {
                var vday   = val.substring(0,2)  - 0;
                var vmonth = val.substring(3,5)  - 1; // because months in JS start with 0
                var vyear  = val.substring(6,10) - 0;
                var tday   = title.substring(0,2)  - 0;
                var tmonth = title.substring(3,5)  - 1; // because months in JS start with 0
                var tyear  = title.substring(6,10) - 0;
                vmSeconds = (new Date(vyear, vmonth, vday)).getTime();
                tmSeconds = (new Date(tyear, tmonth, tday)).getTime();
                // initialize Date() object from calculated milliseconds
                if(vmSeconds<=tmSeconds){
                    return true;
                }else {
                    return false;
                }
           }
           var defaults = {type:'text' , regex : /^([A-Za-zñÑáéíóúÁÉÍÓÚ ]+)$/, accordion: '' , index: 0 , maxi : 0 , mini: 0};
           var regextext = /^([A-Za-zñÑáéíóúÁÉÍÓÚ ]+)$/;
           var regexnum = /^\d+$/;
           ...
           var regexaddress = /^([A-Za-zñÑáéíóúÁÉÍÓÚ .0-9\#\)\(\?\¿\'\-\/\_\,\°\-\:\;]+)$/;
           var regexdate = /^(\d{1,2})[./-](\d{1,2})[./-](\d{4})$/;
           var regexhour = /^(\d{1,2})[./:](\d{1,2})$/;
           var regexdateNormal = /^(\d{1,2})[.//](\d{1,2})[.//](\d{4})$/;
           var settings = $.extend(defaults,options);
           switch (settings.type) {
                case 'text':
                    if ( !(regextext.test($(this).val()))) {
                        $(this).addClass('ui-state-error');
                        $(settings.accordion).accordion( "activate" , settings.index );
                        $(this).focus();
                        return false;
                    }else {
                ...
                case 'none':
                    ...
                    break;
                case 'date':
                    if ( !( regexdate.test($(this).val()) && isDate($(this).val()) )) {
                        $(this).addClass('ui-state-error');
                        $(settings.accordion).accordion( "activate" , settings.index );
                        $(this).focus();
                        return false;
                    ...
                    break;
                case 'datenew':
                    if ( !( regexdate.test($(this).val()) && isDate($(this).val()) && mayor($(this).val(),$(this).attr('title')) )) {
                        $(this).addClass('ui-state-error');
                        $(settings.accordion).accordion( "activate" , settings.index );
                        $(this).focus();
                        return false;
                    ...
                    break;
                case 'hour':
                    if ( !( regexhour.test($(this).val()) && $(this).val().length==5 )) {
                        $(this).addClass('ui-state-error');
                        $(this).focus();
                        return false;
                    }else {
                    ...
                    break;
                 case 'dateNormal':
                    if ( !( regexdateNormal.test($(this).val()) && isDateNormal($(this).val()) && mayor($(this).val(),$(this).attr('title')) )) {
                        $(this).addClass('ui-state-error');
                        $(settings.accordion).accordion( "activate" , settings.index );
                        //$(this).focus();
                        return false;
                    ...
                    break;
                   case 'dateNormalSin':
                    if ( !( regexdateNormal.test($(this).val()) && isDateNormal($(this).val()) )) {
                        $(this).addClass('ui-state-error');
                        $(settings.accordion).accordion( "activate" , settings.index );
                        //$(this).focus();
                        return false;
                ...
                case 'number':
                    ...
                    break;
                case 'length':
                    if ( $(this).val().length > settings.maxi || $(this).val().length < settings.mini ) {
                        $(this).addClass('ui-state-error');
                        $(settings.accordion).accordion( "activate" , settings.index );
                        $(this).focus();
                        return false;
